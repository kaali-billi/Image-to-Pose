...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            elif dataset_name == 'ISS_DATA':
                partial = data[0].cuda()
                gt = data[1].cuda()
            elif dataset_name == 'LRO_DATA':
                partial = data[0].cuda()
                gt = data[1].cuda()
                #Cen_par = data[2].cpu().detach().numpy()
                #Cen_gt = data[3].cpu().detach().numpy()
            elif dataset_name == 'JWST_DATA':
                partial = data[0].cuda()
                gt = data[1].cuda()
            elif dataset_name == 'EUCLID_DATA':
                partial = data[0].cuda()
                gt = data[1].cuda()
            elif dataset_name == 'DC_DATA':
                partial = data[0].cuda()
                gt = data[1].cuda()
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if dataset_name == 'PCN' or dataset_name == 'Projected_ShapeNet':
                ...
                partial = data[0].cuda()
                gt = data[1].cuda()
                cip = data[2].detach().cpu().numpy()
                ret = base_model(partial)
                coarse_points = ret[0]
                dense_points = ret[-1]
                if idx >= 98:
                    recon_pc = dense_points.squeeze(0).detach().cpu().numpy()
                    recon_pc = (recon_pc * 68.74836) + cip
                    rpc = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(recon_pc))
                    o3d.io.write_point_cloud(f'test_set\{idx}_recon.pcd', rpc)
                sparse_loss_l1 = ChamferDisL1(coarse_points, gt)
                sparse_loss_l2 = ChamferDisL2(coarse_points, gt)
                dense_loss_l1 = ChamferDisL1(dense_points, gt)
                dense_loss_l2 = ChamferDisL2(dense_points, gt)
