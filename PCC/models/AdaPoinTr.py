...
class AdaPoinTr(nn.Module):
    ...
    def build_loss_func(self):
        self.loss_cdl = ChamferDistanceL1().cuda()
    def get_loss(self, ret, gt, epoch=1):
        ...
        loss_denoised = self.loss_cdl(denoised_fine, denoised_target)
        loss_denoised = loss_denoised * 0.5

        # recon loss
        loss_fine = self.loss_cdl(pred_fine, gt)
        loss_coarse = self.loss_cdl(pred_coarse, gt)
        loss_recon = loss_fine + loss_coarse
